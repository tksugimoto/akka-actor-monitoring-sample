  ...
  def receive = {
  	case Initialize => 
	    log.info("In PingActor - starting ping-pong")
			...
			val cancel: Cancellable = context.system.scheduler.schedule(initialDelay = 30 seconds, interval = 20 milliseconds) {
				counter += 1
				pongRouter ! PingMessage("ping", counter)
			} (context.system.dispatcher)
			context.system.scheduler.scheduleOnce(delay = 60 seconds) {
				cancel.cancel()
			} (context.system.dispatcher)
